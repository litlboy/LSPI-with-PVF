...
class GridWorld:
    def __init__(self, grid=None, render=False):
        self.grid = grid
        ...
        # Numerical state
        self.x = np.linspace(0, 1, self.coord2state.shape[0])
        self.y = np.linspace(0, 1, self.coord2state.shape[1])
    ...
    def reset(self):
        ...
        x_0 = np.random.choice(np.arange(n_states))
        self.state = x_0
        r, c = self.state2coord[x_0]
        x, y = self.x[r], self.y[c]
        num_state = np.array([x, y])
        return num_state
    ...
    def step(self, action):
        ...
        if action not in self.state_actions[self.state]:
            x, y = self.x[r], self.y[c]
            num_next_state = np.array([x, y])
            return num_next_state, 0, False, {}
        if isinstance(self.grid[r][c], numbers.Number):
            x, y = self.x[r], self.y[c]
            num_next_state = np.array([x, y])
            return num_next_state, 0, True, {}
        else:
        ...
        if self.render:
            self.show(self.state, action, next_state, reward)
        
        self.state = next_state
        x, y = self.x[r], self.y[c]
        num_next_state = np.array([x, y])

        return num_next_state, reward, absorb, {}
...
def make_grid(height, width, goals, traps=None, walls=None, doors=None):
